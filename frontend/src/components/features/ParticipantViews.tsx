...
import { Activity, ArrowLeft, Award, BarChart3, CalendarDays, ChevronLeft, CircleCheck, Clock3, FileSpreadsheet, FileText, Flame, Focus, Lightbulb, Medal, MessageCircle, Moon, MoreHorizontal, Play, Sparkles, Target, Timer, Trophy, Users } from "lucide-react";
...
export function StreaksView() {
  ...
  return <>
    ...
    <section className="streak-celebration"><span><Flame size={28} /></span><div><strong>{generalStreakTitle.label}</strong><p>{generalStreakTitle.nextMilestone ? `تبقى ${generalStreakTitle.nextMilestone - generalStreakTitle.days} يومًا للوصول إلى المرحلة التالية.` : "وصلت إلى أعلى مرحلة مسجلة حتى الآن."}</p>{generalStreakTitle.nextMilestone && <div className="streak-next-milestone"><strong>{generalStreakTitle.days} / {generalStreakTitle.nextMilestone} يومًا</strong><ProgressBar value={generalStreakTitle.progress} tone="teal" /></div>}</div><b><Flame size={16} aria-hidden="true" />{streakData.current} يوم</b></section>
