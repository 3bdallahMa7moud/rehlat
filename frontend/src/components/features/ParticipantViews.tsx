...
import { Activity, ArrowLeft, Award, BarChart3, BookOpen, CalendarDays, ChevronLeft, CircleCheck, Clock3, Crown, Droplets, Dumbbell, FileSpreadsheet, FileText, Flame, Focus, Medal, MessageCircle, Minus, Moon, MoreHorizontal, Play, Sparkles, Target, Timer, TrendingDown, TrendingUp, TriangleAlert, Trophy, Users } from "lucide-react";
...
export function TasksView() {
  ...
  const shortcuts = ["quran", "prayer", "reading"].map((type) => tasks.find((task) => task.type === type)).filter((task): task is (typeof tasks)[number] => Boolean(task));
  const activeTasks = tasks.filter((task) => ["running", "paused", "partial"].includes(task.status));
  return <><PageHeader eyebrow="مساحة العمل" title="مهام اليوم" description="اختر المهمة التي تريدها، ثم أكملها من صفحتها الخاصة." actions={<Link href="/focus"><Button variant="secondary"><Focus size={18} />جلسة تركيز</Button></Link>} />{activeTasks.length > 0 && <section className="active-tasks-section"><SectionHeader title="المهام الجارية الآن" description="هذه المهام بدأت بها بالفعل؛ اضغط على أي مهمة للمتابعة من حيث توقفت." action={<Badge tone="primary">{activeTasks.length} مهام</Badge>} /><div className="task-grid active-tasks-grid">{activeTasks.map((task) => <TaskCard task={task} compact openOnClick key={task.id} />)}</div></section>}<div className="tasks-toolbar"><Tabs value={filter} onValueChange={setFilter} tabs={[{ value: "all", label: "كل المهام" }, { value: "morning", label: "مجموعة الصباح" }, { value: "active", label: "قيد التقدم" }]} /><div className="task-shortcuts">{shortcuts.map((task) => <Link key={task.id} href={`/tasks/${task.id}`}>{activityMeta[task.type].title}</Link>)}</div></div>{shownTasks.length ? <div className="task-grid">{shownTasks.map((task) => <TaskCard task={task} openOnClick key={task.id} />)}</div> : <EmptyState title="لا توجد مهام ضمن هذا العرض." description="جرّب تغيير الفلتر أو ابدأ رحلة اليوم." />}</>;
}
